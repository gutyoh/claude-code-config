# setup-v2.ps1
# Path: claude-code-config/setup-v2.ps1
#
...
if ($Help) {
    Write-Status "Usage: .\setup-v2.ps1 [options]"
    Write-Status ""
    ...
    Write-Status "  .\setup-v2.ps1                     # Interactive mode (recommended)"
    Write-Status "  .\setup-v2.ps1 -Yes                # Full install, no prompts"
    Write-Status "  .\setup-v2.ps1 -Yes -NoMcp         # Full install without MCP servers"
    Write-Status "  .\setup-v2.ps1 -Yes -Mcp brave-search  # Only install Brave Search MCP"
    Write-Status "  .\setup-v2.ps1 -Yes -Minimal       # Core only (hooks, scripts)"
    Write-Status "  .\setup-v2.ps1 -Yes -Theme colorblind  # Full install with colorblind theme"
    Write-Status "  .\setup-v2.ps1 -Yes -BarStyle block -BarPctInside -Components model,usage,cost"
    Write-Status "  .\setup-v2.ps1 -OverwriteSettings  # Interactive, but force-overwrite settings.json"
    exit 0
}
